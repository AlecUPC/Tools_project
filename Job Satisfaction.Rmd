...
## Satisfaction vs Age
```{r}
ggplot(dataset, aes(fill=job_satisfaction, x=age)) +
  geom_bar(position="fill") + 
  xlab("Age") +
  ylab("Percentage of Job Satisfaction")
```
When the job satisfaction is compared with every age, the graph shows that there's not a big difference. Of course there're some differences with each age, but there're not that different to eachother, that the company needs to change their approach to different age groups.There're some outliers, but these are more by coincidence.
...
```
Seeing the summary of the age for each job satisfaction level, it shows that better that there isn't much difference between the levels. Younger employees aren't happier than older employees and vice versa. 
